    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            if len(sequence) == 30:
                ...
                # print(res)
                print(time.time() - start_time, actions[np.argmax(res)], np.max(res))
...
# 解决报错：NotImplementedError: Cannot convert a symbolic Tensor (lstm/strided_slice:0) to a numpy array. This error may indicate that you're trying to pass a Tensor to a NumPy call, which is not supported
# C:\ProgramData\Anaconda3\envs\tf2\Lib\site-packages\tensorflow\python\ops\array_ops.py
